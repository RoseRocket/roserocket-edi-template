...
let { PORT = 5100, HOST = 'localhost', TITLE, PID_FILE } = process.env;
...
if (process.argv.length > 4) {
    fs.writeFile(process.argv[4], process.pid, (err)=> { if(err) console.log(err); });
} else if (PID_FILE) {
    fs.writeFile(PID_FILE, process.pid, (err)=> { if(err) console.log(err); });
}
